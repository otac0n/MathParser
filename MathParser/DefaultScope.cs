...
namespace MathParser
{
    ...
    public class DefaultScope
    {
        ...
        public static Scope Instance { get; } = new Scope(StringComparer.InvariantCultureIgnoreCase)
        {
            ...
            { typeof(double) },
            { typeof(Complex) },
            { Expression.MakeMemberAccess(null, typeof(Complex).GetMember(nameof(Complex.One)).Single()), WKC.One },
            ...
            { (double x) => Math.ReciprocalEstimate(x), WKF.Arithmetic.Reciprocal },
            { (Complex x) => Complex.Reciprocal(x), WKF.Arithmetic.Reciprocal },
